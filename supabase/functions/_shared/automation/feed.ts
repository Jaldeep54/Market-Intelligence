// Deno port of src/lib/automation/feed.ts -- identical logic. The only
// change is the fast-xml-parser import, which uses Deno's npm: specifier
// instead of a bare package name. Everything else here (fetch,
// AbortController, URL) is a Web-standard API available in both Node and
// Deno, so the parsing/fetching logic itself is unchanged. Keep both copies
// in sync if feed handling changes.
